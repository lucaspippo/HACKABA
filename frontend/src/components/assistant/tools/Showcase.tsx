  ...
  return (
    <section className="rounded-xl border border-linea/70 bg-papel/50 p-4">
      <header className="mb-3 flex flex-wrap items-baseline gap-x-3 gap-y-1">
        <h2 className="text-[0.95rem] font-medium text-tinta">{id}</h2>
        <code className="rounded bg-papel-hondo px-1.5 py-0.5 text-[0.78rem] text-tinta-suave">
          {fixture.toolName}
        </code>
        ...
        <button
          ...
          className="ml-auto rounded border border-linea/70 px-2 py-0.5 text-[0.78rem] text-tinta-suave hover:text-tinta"
        >
      ...
      {showJson && (
        <pre className="mt-3 max-h-80 overflow-auto rounded-lg bg-papel-hondo p-3 text-[0.72rem] leading-relaxed text-tinta-suave">
...
export default function Showcase() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h1 className="text-[1.4rem] font-medium text-tinta">Tool presenters</h1>
      <p className="mt-1 max-w-2xl text-[0.88rem] text-tinta-suave">
      ...
      {uncovered.length > 0 && (
        <p className="mt-3 rounded-lg border border-oro/40 bg-oro/10 px-3 py-2 text-[0.82rem] text-oro-tinta">
