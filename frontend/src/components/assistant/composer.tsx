    ...
    >
      ...
      <span className="ms-1 flex w-5 items-center justify-end">
        {attachment.state === "uploading" ? (
          ...
          <CheckIcon className="size-3.5 text-salvia" />
        ) : null}
      </span>
      {attachment.state === "uploading" && (
        <span
          aria-hidden
          className="absolute inset-x-0 bottom-0 h-0.5 bg-violeta/70 transition-[width] duration-300"
  ...
  return (
    ...
    >
      {recording && (
        <span
          aria-hidden
          className="size-1.5 animate-pulse rounded-full bg-violeta motion-reduce:animate-none"
        />
      )}
        ...
        <div className="flex items-baseline justify-between">
          ...
          <p
            className={cn(
              mono,
              "tabular-nums",
              warn ? "text-rojo" : "text-foreground/35",
            )}
          >
