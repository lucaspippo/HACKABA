      ...
      <div className="w-full max-w-sm">
        <div className="mb-7 flex flex-col items-center text-center">
          ...
          <div className="mt-3 flex items-center gap-2">
            ...
            {!resuelto ? (
              ...
              <span className="font-display text-base font-bold text-hielo">{empresa}</span>
            )}
          </div>
        </div>
        ...
        <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-6 sombra-alta">
          <div className="mb-5 flex items-center gap-3">
            <AngelaMark size={36} />
            <div>
              <p className="font-display text-lg font-bold leading-tight">{t("login.titulo")}</p>
          ...
          <form onSubmit={entrar} className="space-y-3">
            <div>
              <label className="mb-1 block text-xs font-semibold text-tinta-suave">{t("login.usuario")}</label>
              <input
                value={username}
                ...
                className="w-full rounded-xl border border-linea bg-papel px-3.5 py-2.5 text-base outline-none focus:border-tinta/40"
              />
            </div>
            <div>
              <label className="mb-1 block text-xs font-semibold text-tinta-suave">{t("login.password")}</label>
              <input
                type="password"
                value={password}
            ...
            {error && (
              <p className="rounded-lg bg-rojo/10 px-3 py-2 text-sm font-medium text-rojo-hondo">{error}</p>
            )}

            <button
              type="submit"
              disabled={cargando || !username || !password}
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-tinta px-4 py-2.5 text-base font-semibold text-crema transition-transform active:scale-[0.99] disabled:opacity-40"
            >
