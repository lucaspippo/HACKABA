...
const GRUPOS_NAV = [
  ...
  },
  {
    ...
    lk: "nav.grupo_cobrar",
    icon: HandCoins,
    ids: ["cuentas", "cobranzas"],
  },
  {
    id: "inventario",
    lk: "nav.grupo_inventario",
    icon: Warehouse,
    ids: [
      "inventario",
      "productos",
  ...
  {
    id: "ingesta",
    lk: "nav.grupo_ingesta",
    icon: Inbox,
    ids: ["cargar", "conectores", "staging", "imported", "saneamiento"],
  },
  {
    id: "compras",
    lk: "nav.grupo_compras",
    icon: ShoppingCart,
    ids: ["proveedores", "ordenes_compra"],
  },
  {
    id: "equipo",
    lk: "nav.grupo_equipo",
    icon: Users,
    ids: ["equipo", "administracion"],
  },
  {
    id: "sistema",
    lk: "nav.grupo_sistema",
    icon: Settings,
    ids: ["documentos", "auditoria", "admin_contexto"],
  },
];
...
function grupoDe(seccionId) {
  for (const g of GRUPOS_NAV)
    if (!g.leaf && g.ids.includes(seccionId)) return g.id;
  return null;
}
...
function DesktopAppInner({ data, oportunidades, fase, user, onRecargar }) {
  ...
  if (
    user.features.includes("inventario") &&
    (user.features.includes("cargar") ||
      user.features.includes("conectores") ||
      user.features.includes("saneamiento"))
  ) {
    extraNav.push("imported");
  }
  if (
    user.features.includes("alertas") ||
    user.features.includes("oportunidades")
  ) {
  ...
  const ALIAS_SECCION = {
    ...
    reparto: "deposito",
    envios: "deposito",
    gestion_equipo: "equipo",
    "gestion de equipo": "equipo",
    alertas: "prioridades",
  ...
  if (vistaHerramienta && !secciones.includes("panel"))
    secciones.unshift("panel");
  const grupos = GRUPOS_NAV.map((g) =>
    g.leaf ? g : { ...g, ids: g.ids.filter((id) => secciones.includes(id)) },
  ...
  const inicial = vistaHerramienta
    ? secciones.includes("panel")
      ? "panel"
      : secciones[0] || "perfil"
    : focoCanon && secciones.includes(focoCanon)
      ? focoCanon
      : secciones[0] || "perfil";
  ...
  const sectionIsValid =
    !!resolvedSection &&
    !!CATALOGO[resolvedSection] &&
    (featuresEfectivas.includes(resolvedSection) ||
      (resolvedSection === "panel" && vistaHerramienta));
  ...
  const redirectTo = !sectionIsValid
    ? inicial
    : resolvedSection !== sectionParam
      ? resolvedSection
      : null;
  ...
  const navegar = (sec, hl) => {
    ...
    if (
      destino === "pendientes" ||
      (destino === "saneamiento" && hl === "revision")
    ) {
      destino = "staging";
      hl = null;
    }
    ...
    if (
      destino &&
      (featuresEfectivas.includes(destino) ||
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-papel text-tinta">
      ...
      />
      <aside
        className={`relative flex shrink-0 flex-col border-r border-linea bg-crema/70 transition-[width] duration-200 ${sidebarColapsado ? "w-16" : "w-64"}`}
      >
        <button
          onClick={() =>
            vistaStore.aplicar({ sidebarColapsado: !sidebarColapsado })
          }
          title={t(
            sidebarColapsado ? "nav.expandir_sidebar" : "nav.colapsar_sidebar",
          )}
        ...
        <div
          className={`flex items-center border-b border-linea py-5 ${sidebarColapsado ? "justify-center px-2" : "px-5"}`}
        >
          {!sidebarColapsado && (
            ...
            return (
              <div key={g.id}>
                <ItemNav
                  ...
                  badge={badgeGrupo}
                  label={t(g.lk)}
                  chevron={!sidebarColapsado}
                  chevronAbierto={abierto}
                  to={`/${g.ids[0]}`}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center gap-4 border-b border-linea bg-papel/80 px-6 py-3 backdrop-blur">
          ...
          >
            <AngelaMark
              size={22}
              estado={stagingCount > 0 ? "esperando" : "idle"}
            />{" "}
            Ángela <PanelRightOpen size={15} />
          </button>
          <AccountMenu
            user={user}
            onVerPerfil={() => navegar("perfil", null)}
          />
        </header>
                ...
                section === "panel" && (
                  <div className="mb-5 flex items-start gap-3 rounded-[var(--radius-card)] border border-oro/30 bg-oro/[0.07] p-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {fase.foco &&
                        user.features.includes(fase.foco) &&
                    ...
                    {section === "panel" &&
                      (vistaHerramienta ? (
                        <div className="mx-auto max-w-[1280px]">
                          <MiDia
                            user={user}
                            onAbrirAngela={preguntar}
                            onTarea={(x) => {
                              const k = PREGUNTA_TAREA[x.tipo];
                              if (k) preguntar(t(k));
                            }}
                            onCerrada={onRecargar}
                            onNavegar={navegar}
                          />
                        </div>
                      ) : (
...
<Inventario
                    ...
                    {section === "cargar" && (
                      <CargarDatos
                        user={user}
                        onArchivoCargado={irAPendientes}
                        onPreguntar={preguntar}
                        onAbrirAngela={abrirAngela}
                        onNavigate={navegar}
                      />
                    )}
                    {section === "finanzas" && (
                    ...
                    {section === "documentos" && (
                      <Documentos onPreguntar={preguntar} />
                    )}
                    {section === "cuentas" && (
                      <CuentasCorrientes
                        onPreguntar={preguntar}
                    ...
                    {section === "administracion" && (
                      <Administracion data={data} onPreguntar={preguntar} />
                    )}
                    {section === "deposito" && (
                      <Deposito
                        data={data}
...
<Evolucion
                    ...
                    {section === "movimientos" && (
                      <Movimientos onNavegar={navegar} highlight={highlight} />
                    )}
                    {section === "conciliacion" && (
                      <Conciliacion
                        onPreguntar={preguntar}
                        onNavegar={navegar}
                        puedeMovimientos={user.features.includes("inventario")}
                      />
                    )}
                    {section === "imported" && (
                      <Imported highlight={highlight} onNavigate={navegar} />
                    )}
                    {section === "proveedores" && (
                      <Proveedores highlight={highlight} />
                    )}
                    {section === "ordenes_compra" && (
                      <OrdenesCompra highlight={highlight} />
                    )}
                  </ErrorBoundary>
                </motion.div>
              </AnimatePresence>
...
function ItemNav({
  ...
  return (
    <div
      className="relative"
      onMouseEnter={() =>
        colapsado && setRect(btnRef.current?.getBoundingClientRect())
      }
        ...
        {!colapsado && (
          <>
            ...
            {chevron && (
              <ChevronRight
                size={15}
                className={`shrink-0 text-tinta-suave transition-transform ${chevronAbierto ? "rotate-90" : ""}`}
              />
            )}
          </>
        )}
