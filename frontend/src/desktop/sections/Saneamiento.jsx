    ...
    <div className="space-y-6">
      ...
      {enRevision > 0 && (
        <button type="button" onClick={() => onNavegar?.("staging")}
          ...
          <span className="text-sm text-tinta">{t("saneamiento.go_staging", { n: enRevision })}</span>
      ...
      {resultado && (
        <div className="flex items-start gap-3 rounded-[var(--radius-card)] border border-salvia/40 bg-salvia/[0.07] p-5">
          ...
          <div className="flex-1">
            <p className="text-base leading-snug text-tinta">{resultado.mensaje}</p>
            ...
            <button
              ...
              disabled={trabajando}
              className="mt-2.5 inline-flex items-center gap-1.5 rounded-full border border-linea bg-crema px-3.5 py-1.5 text-sm font-semibold text-tinta-suave hover:text-rojo"
            >
        ...
        <div className="relative flex items-start gap-3 rounded-[var(--radius-card)] border border-hielo/25 bg-hielo-claro p-5">
          ...
          <div className="flex-1 pr-6">
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button onClick={() => exportarDelta("faro")} className="inline-flex items-center gap-1.5 rounded-full bg-tinta px-4 py-2 text-sm font-semibold text-crema">
                <Download size={14} /> {t("saneamiento.delta_faro")}
              </button>
              <button onClick={() => exportarDelta("tango")} className="inline-flex items-center gap-1.5 rounded-full border border-tinta/25 px-4 py-2 text-sm font-semibold text-tinta">
                <Download size={14} /> {t("saneamiento.delta_tango")}
              </button>
              <button onClick={() => exportarDelta("generico")} className="inline-flex items-center gap-1.5 rounded-full border border-linea px-4 py-2 text-sm font-semibold text-tinta-suave">
                <Download size={14} /> {t("saneamiento.delta_generico")}
              </button>
            </div>
            <p className="mt-2 text-xs text-tinta-suave">{t("saneamiento.delta_nota")}</p>
          </div>
        </div>
      )}
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap items-center gap-2">
                        <span className="rounded-full bg-papel-hondo px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-tinta-suave">
                          {impacto}
                        </span>
                        <span className={`text-xs font-semibold uppercase tracking-wide ${col.txt}`}>
                    ...
                    <div className="flex shrink-0 flex-col items-end gap-2">
                      <button
                        ...
                        className="inline-flex items-center gap-2 rounded-full bg-violeta px-4 py-2 text-sm font-semibold text-crema transition-transform active:scale-95"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-full border border-linea px-3.5 py-1.5 text-xs font-semibold text-tinta-suave hover:text-tinta"
                      >
                    ...
                    >
                      ...
                      <div className="mt-3 space-y-1.5">
                        {preview.muestra.map((m) => (
                          <div key={m.codigo} className="flex items-center gap-2 text-sm">
                        ...
                        {preview.cantidad > preview.muestra.length && (
                          <p className="text-xs text-tinta-suave">{t("saneamiento.preview_mas", { n: num(preview.cantidad - preview.muestra.length) })}</p>
                        )}
                      </div>
                    </PanelDecision>
                  )}
      ...
      {anomalias.length > 0 && (
        <section>
          <h2 className="mb-1 mt-2 font-display text-xl font-bold">{t("saneamiento.anomalias_titulo")}</h2>
                ...
                <div className="flex items-stretch">
                  ...
                  <div className="flex-1 p-5">
                    <div className="flex items-baseline gap-2">
                      ...
                      <h3 className="font-display text-lg font-bold">{a.titulo}</h3>
                    </div>
                    <p className="mt-1.5 max-w-2xl text-sm leading-snug text-tinta-suave">{a.descripcion}</p>
                    {a.impacto_pesos > 0 && (
                      <span className="mt-2 inline-block rounded-full bg-papel-hondo px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wide text-tinta-suave">
