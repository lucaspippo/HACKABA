    ...
    <div className="flex items-start gap-3 rounded-[var(--radius-card)] border border-violeta/15 bg-violeta/[0.04] p-5">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <span className="text-[0.76rem] leading-snug text-tinta-suave">{t("caja.rep_nota")}</span>
        </div>
      </div>
    </div>
  );
...
export default function Caja() {
  ...
  if (!caja) return <p className="text-[0.9rem] text-tinta-suave">{t("caja.cargando")}</p>;
    ...
    <div className="space-y-6">
      <header className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="mt-1 text-[0.9rem] text-tinta-suave">{fecha(caja.fecha)} · {caja.abierta ? t("caja.abierta") : t("caja.cerrada")}</p>
        </div>
      </header>
      ...
      {cierre && (
        <div className="flex items-start gap-3 rounded-[var(--radius-card)] border border-hielo/25 bg-hielo-claro p-5">
          <AngelaMark size={32} />
          <div>
            <p className="text-[1rem] text-tinta">{t("caja.cierre_total")} <b className="plata">{peso(cierre.total)}</b>
            ...
            {cierre.nota_angela && <p className="mt-1 text-[0.88rem] text-tinta-suave">{cierre.nota_angela}</p>}
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-5">
        <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-4 sombra-papel sm:col-span-1">
          ...
          <p className="text-[0.78rem] text-tinta-suave">{t("caja.en_caja_ahora")}</p>
        </div>
        {caja.medios.map((mdio) => (
          <div key={mdio} className="rounded-[var(--radius-card)] border border-linea bg-crema p-4 sombra-papel">
            ...
            <p className="text-[0.78rem] text-tinta-suave">{t(MEDIO_LK[mdio])}</p>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
        <h2 className="mb-3 font-display text-[1.05rem] font-bold">{t("caja.cargar_movimiento")}</h2>
        <div className="flex flex-wrap items-end gap-2">
          <select value={form.tipo} onChange={(e) => setForm({ ...form, tipo: e.target.value })} className="rounded-full border border-linea bg-papel px-3 py-2 text-[0.85rem]">
            <option value="ingreso">{t("caja.tipo_ingreso")}</option>
            <option value="egreso">{t("caja.tipo_egreso")}</option>
          </select>
          <select value={form.medio} onChange={(e) => setForm({ ...form, medio: e.target.value })} className="rounded-full border border-linea bg-papel px-3 py-2 text-[0.85rem]">
            {caja.medios.map((mdio) => <option key={mdio} value={mdio}>{t(MEDIO_LK[mdio])}</option>)}
          </select>
          <input type="number" value={form.monto} onChange={(e) => setForm({ ...form, monto: e.target.value })} placeholder={t("caja.ph_monto")} className="w-32 rounded-full border border-linea bg-papel px-3 py-2 text-[0.85rem] outline-none focus:border-tinta/40" />
          <input value={form.detalle} onChange={(e) => setForm({ ...form, detalle: e.target.value })} placeholder={t("caja.ph_detalle")} className="flex-1 rounded-full border border-linea bg-papel px-3 py-2 text-[0.85rem] outline-none focus:border-tinta/40" />
          <button onClick={agregar} className="inline-flex items-center gap-1.5 rounded-full bg-tinta px-4 py-2 text-[0.85rem] font-semibold text-crema"><Plus size={15} /> {t("caja.agregar")}</button>
        </div>
      </div>

      {/* Movimientos del día */}
      <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
        <h2 className="mb-2 font-display text-[1.05rem] font-bold">{t("caja.movimientos_dia")}</h2>
        <div className="space-y-1">
          <div className="flex justify-between border-b border-linea/60 py-1.5 text-[0.84rem] text-tinta-suave">
            <span>{t("caja.saldo_inicial")}</span><span className="plata">{peso(caja.saldo_inicial)}</span>
          </div>
          {caja.movimientos.map((m, i) => (
            <div key={i} className="flex items-center justify-between border-b border-linea/60 py-1.5 text-[0.86rem]">
      ...
      <div className="flex flex-wrap items-center gap-2 rounded-[var(--radius-card)] border border-linea bg-papel-hondo/40 p-5">
        <span className="text-[0.9rem] text-tinta-suave">{t("caja.cerrar_nota")}</span>
        <div className="ml-auto flex items-center gap-2">
          <input type="number" value={declarado} onChange={(e) => setDeclarado(e.target.value)} placeholder={t("caja.ph_declarado")} className="w-48 rounded-full border border-linea bg-crema px-3 py-2 text-[0.85rem] outline-none focus:border-tinta/40" />
          <button onClick={cerrar} className="inline-flex items-center gap-1.5 rounded-full bg-tinta px-4 py-2 text-[0.85rem] font-semibold text-crema"><Lock size={14} /> {t("caja.cerrar")}</button>
        </div>
      </div>
      ...
      {caja.historial?.length > 0 && (
        <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
          <h2 className="mb-2 font-display text-[1.05rem] font-bold">{t("caja.cierres_anteriores")}</h2>
          <div className="space-y-1">
