  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <p className="mt-1 text-base text-tinta-suave">{t("finanzas.subtitulo")}</p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-2">
        ...
        >
          <div className="flex items-center gap-2 text-hielo">
            <Snowflake size={18} />
            <p className="text-sm font-semibold uppercase tracking-[0.14em]">{t("finanzas.congelado_titulo")}</p>
          </div>
          <p className="plata mt-2 text-4xl font-medium leading-none text-hielo">{peso(resumen.inmovilizado_total)}</p>
          <p className="mt-2 text-base leading-snug text-tinta">
            {t("finanzas.congelado_detalle")}
          </p>
          {onNavegar && (
            <p className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-hielo">
        ...
        <div className="rounded-[var(--radius-card)] border border-salvia/30 bg-salvia/[0.08] p-6 sombra-papel">
          <div className="flex items-center gap-2 text-salvia">
            <TrendingUp size={18} />
            <p className="text-sm font-semibold uppercase tracking-[0.14em]">{t("finanzas.potencial_titulo")}</p>
          </div>
          ...
          {onNavegar && alertas.sin_pvp.cantidad > 0 && (
            <button
              ...
              className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-salvia hover:underline"
            >
      ...
      {liq && (
        <div>
          <h2 className="mb-3 font-display text-lg font-bold">{t("finanzas.liquidez_titulo")}</h2>
          {liq.pagos_vencidos > 0 && (
            <p className="mb-3 rounded-xl border border-rojo/25 bg-rojo/[0.05] px-4 py-2.5 text-base text-tinta">
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
              <div className="flex items-center gap-2 text-oro-tinta">
                <Banknote size={16} />
                <p className="text-sm font-semibold uppercase tracking-[0.12em]">{t("finanzas.liq_por_pagar")}</p>
              </div>
              <p className="plata mt-2 text-3xl font-medium leading-none">{pesoCorto(liq.por_pagar_total)}</p>
              <p className="mt-2 text-base text-tinta-suave">{t("finanzas.liq_por_pagar_sub", { monto: pesoCorto(liq.por_pagar_semana) })}</p>
            </div>
            <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
              <div className="flex items-center gap-2 text-salvia">
                <CreditCard size={16} />
                <p className="text-sm font-semibold uppercase tracking-[0.12em]">{t("finanzas.liq_tarjeta")}</p>
              </div>
              <p className="plata mt-2 text-3xl font-medium leading-none text-salvia">{pesoCorto(liq.tarjeta_7dias)}</p>
              <p className="mt-2 text-base text-tinta-suave">{t("finanzas.liq_tarjeta_sub")}</p>
            </div>
            <div className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
              <div className="flex items-center gap-2 text-hielo">
                <FileText size={16} />
                <p className="text-sm font-semibold uppercase tracking-[0.12em]">{t("finanzas.liq_cheques")}</p>
              </div>
              <p className="plata mt-2 text-3xl font-medium leading-none text-hielo">{pesoCorto(liq.cheques_total)}</p>
              <p className="mt-2 text-base text-tinta-suave">{t("finanzas.liq_cheques_sub", { n: num(liq.cheques_cartera) })}</p>
            </div>
          </div>
        </div>
      )}
      ...
      {pagos?.proyeccion?.disponible && (
        <div>
          <h2 className="mb-1 font-display text-lg font-bold">{t("finanzas.proyeccion_titulo")}</h2>
          <p className="mb-3 text-base text-tinta-suave">{pagos.proyeccion.supuestos}</p>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            {pagos.proyeccion.ventanas.map((v) => (
              <div key={v.dias} className="rounded-[var(--radius-card)] border border-linea bg-crema p-5 sombra-papel">
                <p className="text-sm font-semibold uppercase tracking-[0.14em] text-tinta-suave">
      ...
      {(!hayCuentas || !liq) && (
        <div>
          <h2 className="mb-3 font-display text-lg font-bold">{t("finanzas.desbloquea_titulo")}</h2>
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
            {[
              ...
              <div key={b.tk} className="rounded-[var(--radius-card)] border border-dashed border-linea bg-papel-hondo/40 p-5">
                ...
                <p className="mt-2 font-display text-base font-bold leading-tight">{t(b.tk)}</p>
                <p className="mt-1 text-base text-tinta-suave">{t("finanzas.falta", { que: t(b.fk) })}</p>
              </div>
            ))}
          </div>
        </div>
