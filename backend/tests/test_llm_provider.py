...
in ANTHROPIC_AUTH_TOKEN) must NOT fall through to the deterministic
...
def _clear_llm_env(monkeypatch):
    for var in ("LLM_PROVIDER", "ANTHROPIC_API_KEY", "ANTHROPIC_AUTH_TOKEN",
                "ANTHROPIC_BASE_URL", "ANGELA_MODEL"):
...
def test_explicit_gateway_with_token_selects_gateway(monkeypatch):
    _clear_llm_env(monkeypatch)
    monkeypatch.setenv("LLM_PROVIDER", "gateway")
    monkeypatch.setenv("ANTHROPIC_AUTH_TOKEN", "gw-test-token")
...
def test_autodetect_gateway_token_only_selects_gateway(monkeypatch):
    _clear_llm_env(monkeypatch)
    monkeypatch.setenv("ANTHROPIC_AUTH_TOKEN", "gw-test-token")
    assert config_module.provider() == "gateway"
...
def test_gateway_with_empty_anthropic_api_key_reports_claude_mode(monkeypatch):
    ...
    ANTHROPIC_AUTH_TOKEN — modo() must say 'claude', not 'simulado'."""
    _clear_llm_env(monkeypatch)
...
def test_get_client_uses_auth_token_and_base_url_for_gateway(monkeypatch):
    ...
    client = config_module.get_client()
    assert isinstance(client, _FakeAnthropicClient)
    assert client.kwargs.get("auth_token") == "gw-test-token"
...
def test_get_client_respects_explicit_gateway_base_url(monkeypatch):
    ...
    monkeypatch.setenv("LLM_PROVIDER", "gateway")
    monkeypatch.setenv("ANTHROPIC_AUTH_TOKEN", "gw-test-token")
    monkeypatch.setenv("ANTHROPIC_BASE_URL", "https://custom.gateway.example")
...
# ---------------------------------------------------------------------------
# Model slug resolution: per-provider default, explicit ANGELA_MODEL wins.
# ---------------------------------------------------------------------------
